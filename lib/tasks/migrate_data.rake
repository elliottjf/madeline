namespace :tww do
  ...
  task :migrate_legacy => :environment do
    ...
    puts "steps: #{Legacy::ProjectEvent.count}"
    Legacy::ProjectEvent.where('Type' => 'Paso').each &:migrate
    ProjectStep.connection.execute("SELECT setval('project_steps_id_seq', (SELECT MAX(id) FROM project_steps))")
